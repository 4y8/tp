...
let rec next m state c =
  match m.transition state c with
    None -> next m (m.fail state) c
  | Some s -> s

let adn_of_char = function
    'A' -> A
  | 'C' -> C
  | 'G' -> G
  | 'T' -> T
  | _ -> failwith ""
let recognize_all s m =
  let st = ref m.init in
  let n = String.length s in
  let l = ref [] in
  for i = 0 to n - 1 do
    st := next m !st (adn_of_char s.[i]);
    let add s =
      l := (i - (String.length s) + 1, s) :: !l
    in
    List.iter add (m.output !st)
  done;
  !l

let _ =
  Printf.printf "%d\n" ((List.length @@ recognize_all (read_text "2000/chaine_1000.txt") simple_machine) mod 10000);
  Printf.printf "%d\n" ((List.length @@ recognize_all (read_text "2000/chaine_5000000.txt") simple_machine) mod 10000)
